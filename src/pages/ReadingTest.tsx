...
import { ArrowLeft, Clock, AlertCircle, Loader2, CheckCircle2, XCircle } from 'lucide-react';
import { supabase } from '../lib/supabase';
import { ReadingTest as ReadingTestType } from '../types/reading';
...
export default function ReadingTest() {
  ...
  useEffect(() => {
    const fetchTest = async () => {
      ...
      if (error) {
        ...
        const content = typeof data.question_content === 'string' ? JSON.parse(data.question_content) : data.question_content;
        
        // Map external passage columns into the JSON structure if they exist
        if (content.passages) {
          if (data.passage_1 && content.passages[0]) content.passages[0].text = data.passage_1;
          if (data.passage_2 && content.passages[1]) content.passages[1].text = data.passage_2;
          if (data.passage_3 && content.passages[2]) content.passages[2].text = data.passage_3;
        }
      ...
      interval = setInterval(() => {
        setCountdown((prev) => {
          if (prev <= 1) {
            ...
            setTestStarted(true);
            setTimer(60 * 60); // 60 minutes for reading test
            return 0;
          }
          return prev - 1;
        });
  ...
  const confirmSubmit = async () => {
    ...
    if (session) {
      const timeSpent = mode === 'test' ? (60 * 60) - timer : timer;
      ...
      try {
        ...
        const { error: submissionError } = await supabase.from('reading_submissions').insert({
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-slate-50 pb-24">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-slate-200 shadow-sm px-4 sm:px-6 lg:px-8 py-4">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4 w-full sm:w-auto">
            <button onClick={() => navigate('/skill/reading')} className="text-slate-500 hover:text-slate-900">
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center">
          <div className="bg-white p-8 rounded-2xl shadow-2xl text-center max-w-sm w-full mx-4">
            ...
            <p className="text-sm text-slate-600 mb-6">
              You will have 60 minutes to complete the reading test.
            </p>
            <button
      ...
      {contextMenu.show && (
        <div 
          className="fixed z-50 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden flex flex-col w-32"
          style={{ top: contextMenu.y, left: contextMenu.x }}
        >
          <button 
            onMouseDown={(e) => e.preventDefault()} 
            onClick={() => applyHighlight('#fef08a')} 
            className="px-4 py-2 text-left text-sm hover:bg-slate-100 flex items-center gap-2"
          >
            <div className="w-3 h-3 rounded-full bg-yellow-200"></div> Yellow
          </button>
          <button 
            onMouseDown={(e) => e.preventDefault()} 
            ...
            className="px-4 py-2 text-left text-sm hover:bg-slate-100 flex items-center gap-2"
          >
            <div className="w-3 h-3 rounded-full bg-green-200"></div> Green
          </button>
          <button 
            onMouseDown={(e) => e.preventDefault()} 
            ...
            className="px-4 py-2 text-left text-sm hover:bg-slate-100 flex items-center gap-2"
          >
            <div className="w-3 h-3 rounded-full bg-pink-200"></div> Pink
          </button>
          <div className="h-px bg-slate-200 my-1"></div>
          <button 
            onMouseDown={(e) => e.preventDefault()} 
            onClick={() => applyHighlight('transparent')} 
            className="px-4 py-2 text-left text-sm hover:bg-slate-100 text-slate-600"
          >
            Clear
          </button>
        </div>
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {isSubmitted && (
          <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-6 mb-8 text-center shadow-sm">
            ...
            <button
              onClick={() => navigate('/skill/reading')}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-1/2 bg-white rounded-xl shadow-sm ring-1 ring-slate-200 p-6 sm:p-8">
            <h2 className="text-2xl font-bold text-slate-900 mb-6">{test.content.passages[activePassageIndex].title}</h2>
            <div 
              ref={passageRef}
              className="prose prose-slate max-w-none text-slate-700 leading-relaxed outline-none"
              ...
              dangerouslySetInnerHTML={{ __html: test.content.passages[activePassageIndex].text.replace(/\n/g, '<br/>') }}
            />
          </div>
          ...
          <div className="w-full lg:w-1/2 flex flex-col">
            ...
            <div className="flex overflow-x-auto border-b border-slate-200 mb-6 hide-scrollbar shrink-0">
              <div className="flex space-x-8 px-2">
                {test.content.passages.map((passage, index) => (
                  <button
                    key={passage.id}
            ...
            <div className="bg-white rounded-xl shadow-sm ring-1 ring-slate-200 p-6 sm:p-8 flex-1">
              {test.content.passages[activePassageIndex].sections.map((section) => (
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            {q.options?.map((opt, i) => {
                              ...
                              let optionClass = "flex items-center p-3 rounded-lg border-2 transition-all cursor-pointer ";
                              ...
                              if (isSubmitted) {
                                if (isCorrectOption) {
                                  ...
                                  optionClass += "border-slate-200 bg-white text-slate-500 opacity-50";
                                }
                              } else {
